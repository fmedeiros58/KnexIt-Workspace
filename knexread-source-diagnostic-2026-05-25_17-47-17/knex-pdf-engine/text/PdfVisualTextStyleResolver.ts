...
function getFontDescriptorForWeightAndStyle(block: VisualTextBlockMetadata): string {
...
function inferVisualRole(block: VisualTextBlockMetadata): KnexPdfVisualTextRole {
  ...
  if (fontSize >= 17 || (isUpper && trimmedLength > 8)) return "heading";
  if (fontSize <= 9) return "caption";

  return "body";
}

function classifyFontFamily(candidates: string[]): "sans" | "serif" | "mono" | "symbol" | "unknown" {
...
function fallbackForFamilyKind(kind: "sans" | "serif" | "mono" | "symbol" | "unknown"): string {
...
function resolveFontWeight(
  ...
  if (explicit === "500" || descriptor.includes("medium") || descriptor.includes("semibold")) {
    return "500";
  }
  ...
  /**
   * Não engrossar automaticamente títulos quando a fonte real não foi extraída.
   * O espessamento artificial altera largura, quebra alinhamento e piora a
   * coincidência com as caixas vindas do PDFium.
   */
...
function resolveLineHeight(
  ...
  if (explicitLineHeight > 0) {
    return clampNumber(
      explicitLineHeight,
      Math.max(1, fontSize * 0.9),
      Math.max(1, fontSize * 1.28),
      fontSize * 1.05,
    );
  }
  ...
  return Math.max(1, fontSize * 1.05);
}
...
function resolveLetterSpacing(block: VisualTextBlockMetadata): number | undefined {
  ...
  return clampNumber(value, -1.5, 4, 0);
}
...
export function resolveKnexPdfVisualTextStyle(
  ...
  const visualRole = visualBlock.visualRole ?? inferVisualRole(visualBlock);
