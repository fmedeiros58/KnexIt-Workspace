...
npm run dev
```
Configure as variáveis `JWT_SECRET`, `SSO_PUBLIC_URL`, `CORE_DATABASE_URL` e `PORT`. O serviço publica JWKS em `/auth/.well-known/jwks.json` para que `portal`, `vioclass`, `supadrive` e `viohub` validem tokens.
