...
export function retryReconnectHandler(
  input: RetryReconnectHandlerInput = {},
): RetryReconnectHandlerOutput {
  const maxAttempts = Number.isFinite(input.maxAttempts)
    ? Math.max(1, Math.trunc(input.maxAttempts as number))
    : 5;
  ...
  const jitterMs = Number.isFinite(input.jitterMs)
    ? Math.max(0, Math.trunc(input.jitterMs as number))
    : 250;
