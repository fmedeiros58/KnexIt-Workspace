...
export function handoffPreparatoryToReflective(state: ProcessingState): ProcessingState {
  ...
  state.preparatoryState = {
    ...state.preparatoryState,
    goal: sanitizeText(state.preparatoryState.goal),
    constraints: sanitizeStringArray(state.preparatoryState.constraints, 16),
    ambiguityFlags: sanitizeStringArray(state.preparatoryState.ambiguityFlags, 12),
    salientTerms: sanitizeStringArray(state.preparatoryState.salientTerms, 16),
    cognitivePlan: sanitizeStringArray(state.preparatoryState.cognitivePlan, 24),
  };

  state.executionPlan.steps = sanitizeStringArray(state.executionPlan.steps, 32);
