...
function buildFocusedFactualQuery(query: string): string | null {
...
export async function runKnowledgeLayer(state: ProcessingState): Promise<ProcessingState> {
  ...
  const focusedQuery = buildFocusedFactualQuery(query);
  ...
  const retrieved = dedupeKnowledgeCandidates(runRetriever(retrievalQuery, baseCandidates, 8));
  ...
  const webResultsPrimary = useWeb ? dedupeKnowledgeCandidates(await searchWebFallback(retrievalQuery)) : [];
  const webResultsFocused =
    useWeb && focusedQuery && focusedQuery !== retrievalQuery
      ? dedupeKnowledgeCandidates(await searchWebFallback(focusedQuery))
      : [];
  const webResults = dedupeKnowledgeCandidates([...webResultsPrimary, ...webResultsFocused]);
  ...
  state.trace.push(
    makeTraceEvent({
      ...
      route: state.executionPlan.selectedRoute,
      latencyMs: Date.now() - startedAt,
      detail: `signature=${querySignature}; sources=${selected.length}; citations=${citations.length}; web=${useWeb}; focusedQuery=${focusedQuery ? "true" : "false"}; confidence=${evidenceConfidence.toFixed(3)}`,
    }),
  );
