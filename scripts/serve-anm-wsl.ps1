...
if (-not (Get-Command wsl -ErrorAction SilentlyContinue)) {
  throw "WSL nao encontrado. O script serve:anm requer WSL habilitado."
}
...
if ([string]::IsNullOrWhiteSpace($WorkspacePath)) {
  throw "Nao foi possivel resolver workspace no WSL. Defina ANM_WSL_WORKSPACE_DIR."
}

$entryScriptPath = if (-not [string]::IsNullOrWhiteSpace($env:ANM_WSL_ENTRY_SCRIPT)) {
  $env:ANM_WSL_ENTRY_SCRIPT
} else {
  $EntryScript
}
if ([string]::IsNullOrWhiteSpace($entryScriptPath)) {
  throw "Entry script do ANM vazio. Defina ANM_WSL_ENTRY_SCRIPT ou use -EntryScript."
}
