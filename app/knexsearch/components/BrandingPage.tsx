    ...
    <main className="bg-white text-slate-900">
          ...
          <div className="md:w-1/2">
            <div className="rounded-3xl border border-slate-200 bg-white shadow-lg p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold text-slate-900">{demo?.title ?? "Exemplo"}</div>
                {demo?.statusLabel && (
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${
                      demo.statusTone ? toneClass[demo.statusTone] : "bg-indigo-50 text-indigo-700"
                    }`}
                  >
                    {demo.statusLabel}
                  </span>
              ...
              {demo?.sections.map((sec) => (
                <div key={sec.title} className="rounded-2xl border border-slate-200 bg-indigo-50/60 p-4 space-y-2">
              ...
              {!demo && (
                <div className="rounded-2xl border border-slate-200 bg-indigo-50/60 p-4 space-y-2">
                  ...
                  <p className="text-sm text-slate-700">Exemplo interativo do produto será exibido aqui.</p>
                </div>
              )}
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 md:px-6">
        <div className="space-y-3 text-center">
          ...
          <p className="text-lg text-slate-600">Integrações nativas com aulas, arquivos, IA e fluxos.</p>
        </div>
