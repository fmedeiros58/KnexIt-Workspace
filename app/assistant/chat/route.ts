...
export async function POST(req: NextRequest) {
  ...
  logger.info("ASSISTANT_CHAT_REQUEST", {
  ...
  try {
    ...
    if (!message) {
      throw new RagPipelineError(
        ...
        "Envie message (ou question/prompt) para executar /assistant/chat.",
      );
    }
    if (message.length > MAX_MESSAGE_CHARS) {
      throw new RagPipelineError(
        400,
        "ASSISTANT_CHAT_MESSAGE_TOO_LONG",
    ...
    if (normalizedHistory.droppedCount > 0 || normalizedHistory.truncatedCount > 0) {
      logger.warn("ASSISTANT_CHAT_HISTORY_SANITIZED", {
        requestId: context.requestId,
    ...
    if (wantsStream) {
      ...
      logger.info("ASSISTANT_CHAT_STREAM_OPEN", {
        requestId: context.requestId,
        path: context.path,
        streamMode,
      });
    ...
    if (error instanceof RagPipelineError) {
      logger.warn("ASSISTANT_CHAT_ERROR", {
        requestId: context.requestId,
        code: error.code,
        status: error.status,
      });
      const safeMessage =
        error.status >= 500
          ? sanitizePublicErrorMessage(error.message, "Falha interna ao executar /assistant/chat.")
          : error.message;
      return jsonWithCors(
        context,
    ...
    const message = error instanceof Error ? error.message : "Falha interna ao executar /assistant/chat.";
    logger.error("ASSISTANT_CHAT_INTERNAL_ERROR", { requestId: context.requestId });
    return jsonWithCors(
      context,
      {
        ok: false,
        code: "ASSISTANT_CHAT_INTERNAL_ERROR",
        message: sanitizePublicErrorMessage(message, "Falha interna ao executar /assistant/chat."),
      },
      500,
      { methods: ROUTE_OPTIONS.methods },
    );
