...
type LoginPageClientProps = {
  routeProductSlug?: string | null;
};

const DEFAULT_PRODUCT = getProduct(DEFAULT_PRODUCT_SLUG)!;

export default function LoginPageClient({ routeProductSlug }: LoginPageClientProps) {
  ...
  const { targetRedirect, productLabel, productSlug } = useMemo(() => {
    ...
    if (typeof window === "undefined") {
      const serverProductSlug = routeProductSlug?.toLowerCase() ?? DEFAULT_PRODUCT.slug;
      const serverProduct = getProduct(serverProductSlug) ?? DEFAULT_PRODUCT;
      const serverFallback =
        getProductBaseUrl(serverProduct.slug) || getProductBaseUrl(DEFAULT_PRODUCT.slug) || "http://localhost";
      return {
        targetRedirect: resolveTarget(serverProduct.homePath, serverFallback) ?? serverProduct.homePath,
        productLabel: serverProduct.name,
        productSlug: serverProduct.slug,
      };
    }
    ...
    const routeSlug = routeProductSlug?.toLowerCase();
    const inferredSlug = (() => {
    ...
    const resolvedProduct = getProduct(inferredSlug) ?? DEFAULT_PRODUCT;
    const base =
      getProductBaseUrl(resolvedProduct.slug) ||
      getProductBaseUrl(DEFAULT_PRODUCT.slug) ||
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      document.title = productLabel ? `KNEXIT | Login - ${productLabel}` : "KNEXIT | Identificação";
    }
  }, [productLabel]);
