...
export default function Nav() {
  ...
  const APPS_MAIN: AppItem[] = [
    ...
    { title: "Notícias", href: newsUrl(),    external: true, icon: <IconNews className="h-8 w-8" /> },
  ...
  const APPS_PRODUCTIVITY: AppItem[] = [
    ...
    { title: "UpConect",  href: "/upconect",                     icon: <IconUpConect className="h-10 w-10" />, size: "lg" },
  ];
  ...
  const APPS_EXTRAS: AppItem[] = [
    ...
    { title: "Apresentações", href: slidesUrl(),    external: true, icon: <IconSlides className="h-8 w-8" /> },
    { title: "Formulários",   href: formsUrl(),     external: true, icon: <IconForms className="h-8 w-8" /> },
      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-slate-200">
        ...
        <div className="mx-auto w-full max-w-7xl px-6 pb-2 mt-3 md:mt-6">
          <nav className="w-full flex flex-wrap justify-between gap-x-3 md:gap-x-4 gap-y-2 text-[16px] md:text-[17px]">
            ...
            {/* Substituição: Blog -> Pergunte à Letícia */}
            <NavItem href="/leticia" active={isActive("/leticia")}>Pergunte à Letícia</NavItem>
          </nav>
        </div>
      </nav>
            ...
            <div className="relative" data-profile-popover>
              ...
              {profileOpen && (
                ...
                >
                  ...
                  <div className="p-2">
                    <Link href="/dashboard" className="block rounded-lg px-3 py-2 text-sm text-slate-800 hover:bg-slate-50 no-underline">Minha área</Link>
                    <Link href="/settings"  className="block rounded-lg px-3 py-2 text-sm text-slate-800 hover:bg-slate-50 no-underline">Configurações</Link>
                    <button
                      type="button"
                      onClick={async () => {
  ...
  return (
    <div className="px-3 py-3">
      <div className="grid grid-cols-3 gap-x-3 gap-y-3">
        {apps.map((app) => {
          const sizeCls =
            app.size === "lg" ? "h-14 w-14" :
            app.size === "sm" ? "h-10 w-10" :
                                "h-12 w-12";
          ...
          const Label = (
            <span className="text-[12px] leading-4 text-slate-700 text-center">{app.title}</span>
          );
...
function AvatarSquare({
  ...
  return (
    <span
      ...
      aria-hidden
    >
      <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke={color} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round">
...
/* ====== Ícones Google-like ====== */
function IconGoogleG({ className = "" }: { className?: string }) { return (
  <svg viewBox="0 0 24 24" className={className} aria-hidden>
    <path fill="#4285F4" d="M22 12.2c0-.7-.1-1.4-.2-2H12v3.8h5.6c-.2 1.3-.9 2.4-2 3.2v2.7h3.3c1.9-1.7 3.1-4.2 3.1-7.7Z"/>
    <path fill="#34A853" d="M12 22c2.7 0 5-0.9 6.6-2.5l-3.3-2.7c-.9.6-2 .9-3.3.9-2.6 0-4.7-1.8-5.5-4.2H3.1v2.7C4.8 19.8 8.1 22 12 22Z"/>
    <path fill="#FBBC05" d="M6.5 13.5a6 6 0 0 1 0-3.1V7.7H3.1A10 10 0 0 0 2 12a10 10 0 0 0 1.1 4.3l3.4-2.8Z"/>
    <path fill="#EA4335" d="M12 6.1c1.5 0 2.8.5 3.9 1.5l2.9-2.9A10 10 0 0 0 12 2 10 10 0 0 0 3.1 7.7l3.4 2.7C7.3 7.9 9.4 6.1 12 6.1Z"/>
  </svg>
); }
function IconYouTube({ className = "" }: { className?: string }) { return (
  <svg viewBox="0 0 24 24" className={className} aria-hidden>
    <path fill="#FF0000" d="M23 7.5a4 4 0 0 0-2.8-2.8C18.3 4.3 12 4.3 12 4.3s-6.3 0-8.2.4A4 4 0 0 0 1 7.5 42 42 0 0 0 1 12a42 42 0 0 0 .8 4.5 4 4 0 0 0 2.8 2.8c1.9.4 8.2.4 8.2.4s6.3 0 8.2-.4a4 4 0 0 0 2.8-2.8c.4-1.9.4-4.5.4-4.5s0-2.6-.4-4.5Z"/>
