...
export type KnexPdfPageRotation = 0 | 90 | 180 | 270;

export type KnexPdfPageGeometry = {
  pageNumber: number;

  /**
   * Dimensao base da pagina antes do zoom logico. Normalmente sao pontos PDF.
   */
  baseWidth: number;
  baseHeight: number;

  /**
   * Dimensao visual da pagina em CSS pixels.
   */
  cssWidth: number;
  cssHeight: number;

  /**
   * Dimensao real do bitmap usado pelo canvas.
   */
  bitmapWidth: number;
  bitmapHeight: number;

  /**
   * Zoom logico escolhido pelo usuario.
   * 1 = 100%, 2 = 200%.
   */
  zoom: number;

  /**
   * Device pixel ratio usado como entrada de diagnostico/qualidade.
   */
  devicePixelRatio: number;

  /**
   * Multiplicador de qualidade resolvido para o render.
   */
  qualityScale: number;

  /**
   * Escala final entre bitmap real e caixa CSS.
   */
  outputScale: number;
  outputScaleX: number;
  outputScaleY: number;
  ...
  /**
   * Rotacao normalizada da pagina.
   */
  rotation: KnexPdfPageRotation;

  /**
   * Diagnosticos de clamp/memoria. Nao devem ser usados para layout HTML.
   */
  requestedOutputScale: number;
  requestedMinimumOutputScale: number;
  targetOutputScale: number;
  phaseClampedOutputScale: number;
  maxAllowedOutputScale: number;
  bitmapPixels: number;
...
export type KnexPdfCanvasRenderResult = {
  ...
  /**
   * Fonte unica de geometria quando fornecida pelo backend.
   */
...
export type KnexPdfRenderedPage = {
  ...
  /**
   * Fonte unica de geometria usada para renderizar a pagina.
   */
  geometry?: KnexPdfPageGeometry;
