...
const [rowSub, setRowSub] = useState<"share" | "organize" | null>(null);
  ...
  const listRowLayout =
    "grid items-center gap-4 px-6 py-3 min-w-[960px]";
  const FOLDER_COLORS = [
      ...
      <main className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="relative flex flex-1 min-h-0 h-full items-stretch gap-6 px-4 md:px-6 py-4 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 min-w-0 overflow-y-auto">
            ...
            <div className="rounded-xl border border-slate-200 overflow-x-auto overflow-y-hidden w-full">
              <div
                className={`bg-slate-50 text-slate-600 text-xs w-full ${listRowLayout}`}
                style={{ gridTemplateColumns: listRowCols }}
              >
                <div />
                ...
                <span className="text-right font-medium whitespace-nowrap truncate">Proprietário</span>
                <span className="text-right font-medium whitespace-nowrap truncate">Data da modificação</span>
                <span className="text-right font-medium whitespace-nowrap truncate">Tamanho</span>
                <span ref={sortWrapRef} className="relative flex items-center justify-end text-right font-medium whitespace-nowrap truncate">
              ...
              <ul className="divide-y divide-slate-200 w-full">
                {filtered.map((r) => (
                  <li
                    key={r.id}
                    className={`group w-full ${listRowLayout} hover:bg-sky-100 ${
                  ...
                  >
                    ...
                    <div className="text-sm text-slate-600 text-right truncate min-w-0" title={r.owner || ''}>{r.owner || ''}</div>
                    <div className="text-sm text-slate-600 text-right whitespace-nowrap">{fmtDate(r.createdAt)}</div>
                    <div className="text-sm text-slate-600 text-right whitespace-nowrap">{isFolder(r) ? '-' : fmtSize(r.size)}</div>
      ...
      <div className="flex flex-col items-center gap-3 pointer-events-auto">
        {items.map((item) => (
          <button
            key={item.id}
            type="button"
            className="h-10 w-10 rounded-full bg-white shadow-md ring-1 ring-slate-200 grid place-items-center text-lg hover:shadow-lg hover:ring-slate-300"
