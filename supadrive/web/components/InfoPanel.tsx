...
export function InfoPanel({ title, tabs, emptyMessage, emptyTitle }: InfoPanelProps) {
  return (
    <aside className="h-full w-72 shrink-0">
      <div className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-400">Painel</p>
            <p className="text-base font-semibold text-slate-900">{title}</p>
          </div>
          <button aria-label="close panel" className="text-slate-400">x</button>
        </div>
        <div className="mt-4 flex border-b border-slate-200 text-sm">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`flex-1 border-b-2 px-3 py-2 ${tab.active ? "border-blue-500 font-medium text-blue-700" : "border-transparent text-slate-500"}`}
              type="button"
            >
              {tab.label}
            </button>
